...
SECRET_KEY = require_env('RENTALS_SECRET_KEY')
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                # django.contrib.auth's own AuthenticationMiddleware.
                'django.contrib.auth.context_processors.auth',
                'django.contrib.messages.context_processors.messages',
                'common.context_processors.service_urls',
                # Exposes `user_cache` (this service's own UserCache row for
                # the current user) so templates can show suspension status
                # without any cross-service call - see rentals/context_processors.py.
                'rentals.context_processors.user_cache',
            ],
        },
    },
]
...
if os.environ.get('USE_SQLITE', '1') == '1':
    ...
    DATABASES = {
        'default': {
            ...
            'NAME': os.environ.get('RENTALS_DB_NAME', 'rentals_db'),
...
SESSION_COOKIE_NAME = 'rentals_sessionid'
CSRF_COOKIE_NAME = 'rentals_csrftoken'
...
# Used only by `manage.py bootstrap_caches` (one-time synchronous server-to-
# server hydration of BookCache/UserCache on first startup - see
# rentals/management/commands/). Ongoing sync after that is async, via the
# Celery tasks above. Same hosts as the PUBLIC_URLs above in a standalone dev
# run; kept as separate settings since a real deployment may route internal
# service-to-service calls differently than public/browser-facing URLs.
BOOKS_SERVICE_URL = os.environ.get('BOOKS_SERVICE_URL', BOOKS_SERVICE_PUBLIC_URL)
USERS_SERVICE_URL = os.environ.get('USERS_SERVICE_URL', USERS_SERVICE_PUBLIC_URL)
